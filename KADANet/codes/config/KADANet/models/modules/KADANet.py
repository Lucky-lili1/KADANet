...
class TSLM(nn.Module):
    def __init__(self, nf1, nf2, ksize1=3, ksize2=3, reduction=4, ):
        ...
        self.CA_body2 = MultiFrequencyChannelAttention(
        in_channels=nf2//2,
        dct_h=7,
        dct_w=7,
...
class FSDD(nn.Module):
    def __init__(self, nf1, nf2, ksize1, ksize2, nb, n_B):
        super().__init__()

        self.body = nn.Sequential(*[TSLM(nf1, nf2, ksize1, ksize2, ) for _ in range(nb)],
                                  *[TSGM(nf1, nf2, ksize1, ksize2, ) for _ in range(n_B)],
                                  )
...
class Restorer(nn.Module):
    def __init__(
        ...
        body = [FSDD(nf, nf2, 3, 3, nb, n_B) for _ in range(ng)]

        self.body = nn.Sequential(*body)
        self.fusion = nn.Conv2d(nf + nf2, nf, 3, 1, 1)
        #--------------------

        # self.fusion2 = nn.Conv2d(nf2//2+nf, nf, 3, 1, 1)
        if scale == 4:  # x4
